...
//uploads profile picture to the users collection
  uploadProfilePicture(XFile image, User user, Uint8List? imgdata) async {
    String link = await uploadImage(profilePic, imgdata);
    print('#######################upload post ###############################');
    print(link);
...
//uploads post to the post collection
  uploadPost(XFile? image, String location, String description,
      Uint8List? imgdata) async {
    String link = await uploadImage(posts, imgdata);
    print("-----I am at user post -----\n $link");
